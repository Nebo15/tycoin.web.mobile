var App = (function(){
  ...
  var updateState = function() {
    ...
    if(activities_utime+10000 > new Date().getTime()) {
      return;
    }
  ...
  return {
      ...
      if(use_cache === true && balance) {
        ...
        API.request('GET', '/my/balance')
          .success(function(response) {
            ...
            balance.free_coins_available_time_pretty = Tools.prettyDate(balance.free_coins_available_time);
    ...
    getActivities: function(callback, use_cache) {
      if(use_cache === true && activities) {
        ...
        if($('.activities span.icon-refresh').hasClass('icon-spin')) {
          return;
        }

        $('.activities span.icon-refresh').addClass('icon-spin');

        API.request('GET', '/transaction/history?limit=10')
          .success(function(response) {
    ...
    updateActivities: function(callback) {
      var updateActivitiesList = function(list) {
        ...
        $.each(list, function(index, value) {
          html += Template.compileElement(activity_tpl, value);
        });
