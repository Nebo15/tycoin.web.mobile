...
(function() {
  ...
  $(window).load(function() {
    ...
    $('.backPane').scroll(function(e) {
      ...
      var tolerance = $.isMobile() ? 30 : 0;

      if(!pull_up_end_reached && scrollBottom >= 0) {
        if(!pull_up_active && scrollBottom >= tolerance) {
          $('.pull_up').finish().animate({height: 30});
          pull_up_active = true;
          updateOld();
        }
      }
  ...
  function insertNew(delta) {
    ...
    var addIterated = function() {
      var tmp = delta[delta.length - i];
      if(tmp) {
        var html = Template.compileElement(template, tmp);
        $('.activities .list-divider').after(html);
        i++;
        setTimeout(addIterated, 300);
      }
    };
  ...
  function insertOld(delta) {
    ...
    var addIterated = function() {
      ...
      if(tmp) {
        ...
        i++;
        setTimeout(addIterated, 100);
      } else {
        $('.pull_up').finish().animate({height: 0}, 500, function() {
