var Request = function(method, path, data, params) {
  ...
  var callbacks = {
    ...
    error: [function(jqXHR, textStatus, errorThrown) {
      Loader.show(textStatus + ': ' + errorThrown);
    }],
  ...
  params = $.extend(true, {
    ...
    success: function(data, textStatus, jqXHR) {
      console.log('Request succeeded: ' + textStatus + ' (' + jqXHR.status + ')');

      trigger(callbacks.success, [new Response(data, jqXHR, $this)], jqXHR.status);
    },

    error: function(jqXHR, textStatus, errorThrown) {
      console.log('Request error: ' + textStatus + ' (' + jqXHR.status + ') - ' + errorThrown);
  ...
  return {
    before: function(callback, reset) {
      if(reset) {
        callbacks.before = [];
      }
    ...
    success: function(callback, reset) {
      if(reset) {
        callbacks.success = [];
      }
    ...
    complete: function(callback, reset) {
      if(reset) {
        callbacks.complete = [];
      }
    ...
    statusCode: function(code, callback) {
    ...
    statusCodeGroup: function(codeGroup, callback) {
      if(codeGroup == 2)
