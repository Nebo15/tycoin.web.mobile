var Backpane = (function() {
  ...
  $(window).bind('touchmove', function(e) {
    ...
    if(shift == 0) {
      $('.content, .bar-title, .bar-tab').css("-webkit-transition", "-webkit-transform 0.05s linear");
      $('.content, .bar-title, .bar-tab').css("transition", "transform 0.05s linear");
    }
    ...
    if(shift != 0 || Math.pow(touch.pageX - startTouchX, 2) - Math.pow(touch.pageY - startTouchY, 2) > Math.pow(tolerance, 2)) {
      ...
      if(shift > 0) {
        ...
        if(getBackpaneTransformValue() != 0) {
          if(shift > 0) {
            setBackpaneTransformValue(shift);
          } else {
