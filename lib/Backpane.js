var Backpane = (function() {
  ...
  $(window).bind('touchmove', function(e) {
    ...
    {
      e.preventDefault();
      shift = (touch.pageX - startTouchX)*1.1;

      if(shift > 0)
      {
        if(getBackpaneTransformValue() < viewport_width*85/100)
          setBackpaneTransformValue(shift);
      }
      else
      {
        shift += getBackpaneTransformValue();
  ...
  $(window).bind('touchend', function(e) {
    ...
    {
      e.preventDefault();
      if(viewport_width*30/100 > shift)
      {
        Backpane.hide();
      }
      else
  ...
  return {
    show: function()
    {
      setBackpaneTransformValue(viewport_width*85/100);
