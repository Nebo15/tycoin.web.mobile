var Navigation = {
  ...
  init: function() {
    ...
    $(document).bind('pageRender', function() {
      ...
      // Switch share button between momend and day create
      var current_day = Storage.get('current_day');
      if(current_day !== undefined && current_day.id) {
        $('.tab-item[page=day_create]').attr('page', 'moment_create:source=camera');
      } else {
        $('.tab-item[page=moment_create]').attr('page', 'day_create');
      }
    });
  ...
  redirectToDefaultPage: function() {
    Navigation.redirectToPage('days_interesting');
  },
